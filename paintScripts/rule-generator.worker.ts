import { Rule, FeedBack, RuleType, RuleTags, TagInfo } from '../components/scoring/rule.model';
const jsonpath = require('jsonpath/jsonpath.min.js');
const validationsRepo = require('./validationsRepo.json');
import { MapGenerator } from './map-generator';

const RGWorker: Worker = self as any;

RGWorker.addEventListener("message", (event:any) => {
...
export class RuleGenerator {
    private initialDocJson;
    private finalDocJson;
    constructor(initialDocJson: any, finalDocJson: any) {
        ...
        for (let sheetIndex in this.finalDocJson.sheets) {
            ...
            for (let rowIndex = 0; rowIndex <= rowsCount; rowIndex++) {
                ...
                for (let colIndex = 0; colIndex <= columns; colIndex++) {
                    ...
                    let cellRule: Rule = this.makeCellRule(initialCell, finalCell, sheetIndex, validationList);
    ...
    private makeCellRule(initialCell,finalCell,sheetIndex, validationList) {
        let baseRule:Rule = new Rule(RuleType.SUM);
    ...
    private filterFormatFromValidationList(finalCell, validationList) {
    ...
    private makeValidationRule(baseRule, initialCell, finalCell, validation, sheetId) {
        let validationRule: Rule = new Rule(RuleType.VALIDATION);
    ...
    private makeRuleOrNotOnBasisOfValue(initValue, finalValue) {
        if (initValue !== undefined && finalValue !== undefined) {
    ...
    returnHighestKey(param1: Array<number>, param2: Array<number>) {
        return Math.max.apply(null, Object.keys(param1).concat(Object.keys(param2)))
    }
   
    public generateMappedRules(oldRulesArray:Array<any>,matcherInput) {
        try {       
        let newRulesArray: any = this.generateRules();
